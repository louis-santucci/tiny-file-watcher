...
func TestIntegration_WatcherLifecycle(t *testing.T) {
	...
	_, err := db.CreateMachine("test-machine", "hw-id-lifecycle", "10.0.0.1", 22, "ssh-user", "ssh-key", "/tmp/host.pub")
	require.NoError(t, err)
...
func TestIntegration_ListWatchedFiles(t *testing.T) {
	...
	_, err := db.CreateMachine("test-machine", "hw-id-list", "10.0.0.1", 22, "ssh-user", "ssh-key", "/tmp/host.pub")
	require.NoError(t, err)
...
func TestIntegration_WatcherDeleteCascades(t *testing.T) {
	...
	_, err := db.CreateMachine("test-machine", "hw-id-cascade", "10.0.0.1", 22, "ssh-user", "ssh-key", "/tmp/host.pub")
	require.NoError(t, err)
