...
class FileServiceTests {
    ...
    void testShouldNotPersistFileWhenUnflushedFileExistsInDb() {
        ...
        FileEntity fileEntity = FileEntity
                .builder()
                .id(5L).path("/tmp/test.mp3").filename("test.mp3").isFlushed(false).fileWatcher(fileWatcherEntity).build();
        PanacheQuery<FileEntity> panacheQuery = mock(io.quarkus.hibernate.orm.panache.PanacheQuery.class);
    ...
    void testShouldNotPersistFileWhenFlushedFileExistsInDb() {
        ...
        FileEntity fileEntity = FileEntity
                .builder()
                .id(5L).path("/tmp/test.mp3").filename("test.mp3").isFlushed(true).fileWatcher(fileWatcherEntity).build();
        PanacheQuery<FileEntity> panacheQuery = mock(io.quarkus.hibernate.orm.panache.PanacheQuery.class);
        when(fileRepository.find(anyString(), anyLong(), anyString())).thenReturn(panacheQuery);
        when(panacheQuery.firstResultOptional()).thenReturn(java.util.Optional.of(fileEntity));
